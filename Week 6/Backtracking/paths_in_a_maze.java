...
public class Solution {
    ...
    private static boolean isValidMove(int i, int j, ArrayList<ArrayList<Boolean>> visited,
            ArrayList<ArrayList<Integer>> arr) {
        return (i >= 0 && j >= 0 && i < arr.size() && j < arr.get(i).size() && arr.get(i).get(j) == 1
                && !visited.get(i).get(j));
    }

    private static void allPossiblePaths(int i, int j, StringBuilder currentPath, ArrayList<String> allPaths,
            ArrayList<ArrayList<Boolean>> visited, ArrayList<ArrayList<Integer>> arr) {
        ...
        int n = arr.size();
        // If we have reached the destination cell then add the current path to the
        // answer.
        if (i == n - 1 && j == n - 1) {
    ...
    public static ArrayList<String> findAllPaths(ArrayList<ArrayList<Integer>> arr) {
        ...
        ArrayList<ArrayList<Boolean>> visited = new ArrayList<ArrayList<Boolean>>(n);
